...
def get_ai_reasoning(symbol, price, decision):
    if not client:
        return f"Bullish setup on {symbol} at ${price:,.2f}. HH/HL structure. EMA20 above EMA50. RSI 52. Support at ${price * 0.98:,.2f}. Resistance at ${price * 1.02:,.2f}. Stop Loss: ${price * 0.98:,.2f}. Take Profit: ${price * 1.05:,.2f}. Confidence: 78%."
    try:
...
async def close_position():
    ...
    if position:
        balance = load_balance()
        current_price = fetch_market_data(position["symbol"])
        pl = (current_price - position["entry_price"]) * position["size"] if position["side"] == "BUY" else (position["entry_price"] - current_price) * position["size"]
        new_balance = balance + pl
        save_balance(new_balance)
        save_position(None)
        save_to_journal({"action": "CLOSE_POSITION", "symbol": position["symbol"], "pl": pl, "new_balance": new_balance, "timestamp": datetime.utcnow().isoformat()})
        return HTMLResponse(f"<h2>✅ Position Closed. P/L: ${pl:.2f}</h2><p>New Balance: ${new_balance:,.2f}</p><p><a href='/analyze'>← Back to Dashboard</a></p>")
